...
NSString * const kPSMApplicationJSONType = @"application/json";

NSString * PSMPercentEscapedStringFromString(NSString *string) {
    static NSString * const kPSMCharactersGeneralDelimitersToEncode = @":#[]@";
    static NSString * const kPSMCharactersSubDelimitersToEncode = @"!$&'()*+,;=";

    NSMutableCharacterSet * allowedCharacterSet = [[NSCharacterSet URLQueryAllowedCharacterSet] mutableCopy];
    [allowedCharacterSet removeCharactersInString:[kPSMCharactersGeneralDelimitersToEncode stringByAppendingString:kPSMCharactersSubDelimitersToEncode]];
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[PSMQueryParamCollection class]]) {
        return object;
    }
    else if ([object isKindOfClass:[NSDate class]]) {
        return [PSMSanitizer dateToString:object];
    }
    ...
    else if ([object isKindOfClass:[PSMObject class]]) {
...
- (NSString *) parameterToString:(id)param {
    if ([param isKindOfClass:[NSString class]]) {
        ...
        return [PSMSanitizer dateToString:param];
    }
...
+ (NSString *)dateToString:(id)date {
    NSTimeZone* timeZone = [PSMDefaultConfiguration sharedConfig].serializationTimeZone;
...
- (NSString *) selectHeaderAccept:(NSArray *)accepts {
    ...
    for (NSString *string in accepts) {
        if ([self.jsonHeaderTypeExpression matchesInString:string options:0 range:NSMakeRange(0, [string length])].count > 0) {
            return kPSMApplicationJSONType;
        }
